...
namespace TSviewACD
{
    public class CryptRclone
    {
        ...
        static byte[] dataKey = new byte[32]; // Key for secretbox
        static byte[] nameKey = new byte[32]; // 16,24 or 32 bytes
        static byte[] nameTweak = new byte[nameCipherBlockSize]; // used to tweak the name crypto

        static RNGCryptoServiceProvider rnd = new RNGCryptoServiceProvider();

        static AES_EME name_aes = new AES_EME();
        ...
        public static string DecryptName(string ciphertext)
        {
        ...
        public class CryptRclone_CryptStream : Stream
        {
            ...
            public CryptRclone_CryptStream(Stream baseStream, byte[] nonce = null) : base()
            {
            ...
            public override int Read(byte[] buffer, int offset, int count)
            {
                ...
                while (_position < _cryptedlen)
                {
                    ...
                    if (crypted_blockno != blockno)
                    {
                        ...
                        crypter.Seal(out cryptedBlock, plainBlock, _nonce, dataKey);
        ...
        public class CryptRclone_DeryptStream : Stream, IHashStream
        {
            ...
            public CryptRclone_DeryptStream(Stream baseStream, long orignalOffset = 0, long cryptedOffset = 0, long cryptedLength = -1, byte[] nonce = null) : base()
            {
                innerStream = baseStream;
                if (nonce != null && nonce.Length != fileNonceSize) throw new ArgumentOutOfRangeException("nonce");
                if (nonce != null)
            ...
            private void FillBuffer()
            {
                ...
                if (!decrypter.Open(out plainBlock, cryptedBlock, _nonce, dataKey))
